void pressure_bottom_regularized(
    int Nx, 
    int Ny,
    int Q, 
    int index,
    ...
    for (int i = 0; i < Nx; i++) {
        ...
        // Retrieve bottom quantities
        double rho_bot = val1[i];
        double U_bot = u_inter; 
        double V_bot = v_inter;

        double v_sq_bot = U_bot*U_bot + V_bot*V_bot;
        ...
        for (int k = 0; k < Q; k++) {
            // Compute equilbirium
            double cu_bot = cx[k]*U_bot + cy[k]*V_bot;
            double feq_bot = w[k] * rho_bot * (1.0 + 3.0*cu_bot + 4.5*cu_bot*cu_bot - 1.5*v_sq_bot);
            ...
            f_new[j_bot][i][k] = feq_bot + fneq_reg;
        }
    }
}